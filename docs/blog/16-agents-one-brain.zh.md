...
真正让我有点惊喜的是后来加的 agent mailbox。agent 之间可以 git 原生地异步留言。A 机器上的 agent 干完一段活,给 B 机器的 agent 留一句「这块我改完了,你接着往下」,B 启动时自动收未读。整个过程没有任何消息队列、没有任何服务,就是往仓里写一个文件、push 上去。第一次看到一个 agent 主动读到另一台机器上同事留的言,那感觉挺奇妙的。
