...
- **`queen/limits.example.md`** — an inert template for the per-instance file-size-limit override below. Agents read `queen/limits.md`, not the example, so copying is an explicit act. Shipped as an example rather than a live file on purpose: a live `queen/limits.md` in every instance would freeze that instance's numbers, and since `update.sh` never touches `queen/`, later changes to the §6 protocol defaults would stop reaching it. Includes the reasoning to tune against retrieval quality rather than context-window size, and a tuning log so a deliberate change can be told from a stray edit.
...
- **Directory tree in both READMEs corrected.** It had drifted: `agents/*/carryover/`, `queen/limits.example.md`, `decisions/`, `tests/`, and `scripts/comms/` were all missing, and the per-file script listing was stale enough to mislead. Now shows every top-level layer with the hot/cold distinction on the agent directory.
