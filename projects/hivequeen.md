...
## 一句话
git 原生的 AI agent 上下文协议。fork 即继承，clone 即连接，所有 agent 共用同一个大脑。
...
## 分发模型
用户 fork → clone 到任意机器 → 运行 install 脚本 → 自动接入母体
...
## 待完成
- [ ] Gemini CLI 安装脚本
- [ ] compile.sh 的 LLM 提炼版本（目前是纯文本聚合）
- [ ] 更多工具的软链接安装支持
...
唯一同时具备：git session 同步 + 分层优先级规则 + 多工具支持 + fork 分发模型。
